...
- *origin* é o nome padrão do GitHub quando você faz `git clone`
...
- você não precisa mais escrever `origin feature-login` toda vez  
...
Agora você pode ver as branches remotas com:
...
Se a branch remota já existir localmente, basta trocar:
...
- **fetch** só atualiza a lista de branches remotas (não altera sua branch atual)
